export function getNextOccurrenceForUser(task, userId, fromDate) {
  let cursor = new Date(fromDate);
  cursor.setHours(0, 0, 0, 0);

  for (let i = 0; i < 400; i++) {
    if (doesTaskOccurOnDate(task, cursor)) {
      const occurrenceIndex = getOccurrenceIndex(task, cursor);
      const assignee = getBaseAssignee(task, occurrenceIndex);
