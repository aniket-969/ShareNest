...
const ExpenseContainer = ({ participants }) => {
  ...
  useEffect(() => {
    const handleExpenseTask = (newExpense) => {
      console.log(newExpense, "newie");
      queryClient.setQueryData(["expense", roomId], (oldData) => {
        ...
        updatedPages[0] = {
          ...updatedPages[0],
          expenses: [newExpense, ...updatedPages[0].expenses],
        };
    ...
    socket.on("createdExpense", handleExpenseTask);

    return () => {
      socket.off("createdExpense", handleExpenseTask);
    };
  ...
  const expenses =
    expenseData?.pages?.flatMap((p) => p.expenses).reverse() ?? [];
